# This code is attempting to learn an LSTM model for finding the neighborhood information around fields_of_interest
...
import load_data_StreamSearch as stream_data 
...
class RNNSearch():
	...
	def load_data(self):
		Node_IDs_fname = 'data/StreamSearch_data/field_of_study_ids.tsv'
		#Node_IDs_fname = 'data/StreamSearch_data/node_ids.tsv'
		[self.encode_word, self.decode_word] = stream_data.load_vertex(Node_IDs_fname)		 
	...
	def run_epoch(self, session, dataset, train_op=None, verbose=10):
		...
		if not train_op:
			train_op = tf.no_op()
			dp = 1
		total_steps = sum(1 for x in enumerate(stream_data.get_pathsearch_dataset_batch_varlen(dataset, self.config.batch_size, self.config.max_steps)))
		print total_steps
		total_loss = []
		state = session.run(self.initial_state) 		
		for step, (x, y, dest_ID, seqL) in enumerate(stream_data.get_pathsearch_dataset_batch_varlen(dataset, self.config.batch_size, self.config.max_steps)):
...
def test_RNNSearch():
	...
	with tf.Session() as session:
		...
		for epoch in xrange(config.max_epochs):
			...
			start = time.time()
			###
			#train_pp = model.run_epoch(session, dataset = 'nyt_seqs',train_op=model.train_step) 
			train_pp = model.run_epoch(session, dataset = '1990-94.txt',train_op=model.train_step) 					
			print 'Training perplexity: {}'.format(train_pp)
			if train_pp < best_val_pp:
				best_val_pp = train_pp
				best_val_epoch = epoch
				saver.save(session, './Streamsearch_2layerlstm_1990_94.weights')
			if epoch - best_val_epoch > config.early_stopping:
				break
			print 'Total time: {}'.format(time.time() - start)
		saver.restore(session, 'Streamsearch_2layerlstm_1990_94.weights')		
		...
		f = open("StreamSearch_idx_cluster_2000.txt", "w")
		pk.dump(idx_array, f)
		f.close()
		
		f = open("StreamSearch_Cluster_Centroids_2000.txt", "w")
