...
object GraphMiner {
  ...
  val writerSG2 = new PrintWriter( new File( "GraphMiningOutputSampleHoldSupp2000Degree9.txt" ) )
...
//        gWin.filterNonPatternSubGraphV2( batch_window_intersection_graph.input_graph )
      ...
      val batch_window_intersection_graph = gBatch
      val degree = gBatch.input_graph.degrees
      val sumdegree : Double = degree.map(v=>v._2).reduce((d1,d2) => d1+d2)
      val avg_degree = sumdegree/degree.count
        var level = 0;
...
//      println("pattern type infreqent, promising, closed, redundant, frequent ie. pro+clo+red" , infrequent, " " , promising, " " , closed, " ", redundant, " " , frequent)
      ...
      //frequent_pattern_support_rdd.collect.foreach(p=>println(p._1, " : " , p._2))
      //val p3 =  "/sumitData/work/PhD/PhdResearch/p2pnetowrkdata/p2p-Gnutella05_degree_(.*)_range.*.lg".r
      //val p3(degreeval) = filepath
      val degreeval = 9 
      writerSG2.println(filepath, "\t", degreeval,  "\t", batch_window_intersection_graph.input_graph.edges.count, "\t", frequent_pattern_support_rdd.count)
      writerSG2.flush()
    }
