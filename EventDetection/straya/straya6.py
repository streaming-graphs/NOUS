...
# Set the constants
NUM_WORDS = 1152361  # Highest number used in the normal.npy file
MAX_SENT_LENGTH = 9
MAX_SENTS = 50
EMBEDDING_DIM = 100
...
epochs = sys.argv[1]
attack = np.load(directory + 'attack_ip_pairs_big.npy')
normal = np.load(directory + 'normal_ip_pairs_big.npy')
data = np.vstack((normal, attack))

labels = np.ones([attack.shape[0], 1])
labels = np.vstack((labels, np.zeros([normal.shape[0], 1])))
...
model.fit(x_train, y_train, validation_data=(x_val, y_val), nb_epoch=int(epochs), batch_size=50)
model.save('han_model_straya_ip_pairs_' + epochs + '.h5')

# Evaluate
# pb.push_note('Model evaluation begins', '!!')
scores = model.evaluate(x_test, y_test, verbose=0)[1] * 100
preds = model.predict(x_test)
np.save('preds_straya_ip_pairs_' + epochs + '.npy', preds)
np.save('acts_straya_ip_pairs_' + epochs + '.npy', y_test)
