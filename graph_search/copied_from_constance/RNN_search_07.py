...
# Version v06: Implementation of sequence generation using real-world Graph dataset along with loading wordvectors files in binary format

# This version deals with implementation of 2-layer LSTM cell; Implementation is being carried out on real-world Graph dataset
	...
	with tf.Session() as session:
		...
		for epoch in xrange(config.max_epochs):
			...
			if valid_pp < best_val_pp:
				...
				saver.save(session, './pathsearch_rnnlm_2layer_lstm.weights')
			if epoch - best_val_epoch > config.early_stopping:
				break
			print 'Total time: {}'.format(time.time() - start)
		
		saver.restore(session, 'pathsearch_rnnlm_2layer_lstm.weights')
