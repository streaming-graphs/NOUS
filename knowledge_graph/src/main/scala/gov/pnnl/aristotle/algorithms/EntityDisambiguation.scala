...
import gov.pnnl.aristotle.utils.{TAC_Utils, NYT_Utils}
...
object EntityDisambiguation {
  ...
  def main(args: Array[String]): Unit = {   
    ...
    val allTriples: List[List[Triple]] = ReadTriples.readTriples(args(1), sc)
    ...
    for(triplesInBlock <- allTriples){
      val entityTypeMap = ReadTriples.getEntitiesWithTypeMapFromTriples(triplesInBlock)
      println("No of unique entities in block", entityTypeMap.size)
      entityTypeMap.foreach{v => 
        println(" Calculating entity Disambguation on " + v._1 +"----type--->"+ v._2)  
        val allAnswers = disambiguate(g, v._1 , entityTypeMap.keys.toSet, 0.8, graphNbrs, v._2)
  ...
  def collectiveDisambiguation(g:Graph[String, String], allMentions: List[List[String]], confFile: String, graphNbrs:VertexRDD[Set[String]]): VertexRDD[(String, String, Double)] = {
    
    }
   */

  def disambiguate(g: Graph[String, String], searchForMe : String, queryContextEntities: Set[String], scoreThreshold: Double, graphNbrs:VertexRDD[Set[String]], suggestedEntityType: String = ""): VertexRDD[(String, Double)]= {
    ...
    val nodesWithStringMatchesRDD : RDD[(VertexId, String)] = MatchStringCandidates.getMatchesRDDWithAlias(List(searchForMe), g)
    val nodesWithStringMatches = nodesWithStringMatchesRDD.map(v => v._1).collect()
    ...
    val simScore: VertexRDD[Double] = similarityScore(g, nodesWithStringMatches, queryContextEntities, graphNbrs)  
    ...
    val totalScore: VertexRDD[Double] = tempScore.leftJoin(typeSimScore) {
      case(id, u,v) => u*(wtSimilarity+wtPopularity) + v.get*wtTypeSimilarity 
      case(id, u, None)=> u*(wtSimilarity+wtPopularity)
      } 
   ...
   return final_matches_with_labels_and_score//.top(10)(Ordering.by[(VertexId, (String, Double)), Double](_._2._2))
    
  }
  /*
  def disambiguate(g: Graph[String, String], searchForMe : String, queryContextText: String, sennaConfig: SennaConfig , scoreThreshold: Double, graphNbrs:VertexRDD[Set[String]]): VertexRDD[(String, Double)]= { 
    println("Searching for=" + searchForMe + " in:" + queryContextText )
   
    // Get all candidates matches
    println("Finding candidates for ", searchForMe)
    //val nodesWithStringMatches : Array[Long] = MatchStringCandidates.getMatches(searchForMe, g)
    val nodesWithStringMatchesRDD : RDD[(VertexId, String)] = MatchStringCandidates.getMatchesRDD(searchForMe, g)
    val nodesWithStringMatches = nodesWithStringMatchesRDD.map(v => v._1).collect()
    println("NUm Nodes that match candidate string:" + searchForMe + "=" + nodesWithStringMatchesRDD.count)
    ...
    // 1. Calculate Popularity SCore of each candidate as #incomingLinks/TotalLinks 
    println("Finding popularity of matching candidates")
    val validRelations : String = "linksTo"
    val popularScore: VertexRDD[Double] = popularityScore(g, nodesWithStringMatches, validRelations)
    println("Popular nodes (> 1% connections) and their scores(#neighbours/totalLinks):")
    val score_with_labels = g.vertices.innerJoin(popularScore)((id, u, v)=> (u,v))
    score_with_labels.foreach(v => if(v._2._2 > .01) println(v._2._1 + " Popular score =" + v._2._2))
    ...
    // Combine Scores
    val wtPopularity = 0.25
    val wtSimilarity = 0.75
     
    println("\nCalculating total scores")
    val totalScore: VertexRDD[Double] = simScore.innerZipJoin[Double, Double](popularScore) {(id, u,v) => u*wtSimilarity + v*wtPopularity }
    println("completed score join, final score set size =", totalScore.count) 
   ...
   return final_matches_with_labels_and_score  
  }
  */
  //Do better here
  //Map PER -> person
  // map LOC -> location
  //consider yago type heirachy
  def getTypeSimScore(g:Graph[String, String], candidates: Array[Long], suggestedType: String) : VertexRDD[Double] ={
    val candidateTypes: VertexRDD[String] = NodeProp.getNodeType(candidates, g)
  ...
  def popularityScore(g: Graph[String, String], candidates : Array[Long], validRelations: String) : VertexRDD[Double] = {
    ...
    val popularScore : VertexRDD[Double] =  countWikiLinksOfAllCandidates.mapValues(v => v/totalLinks)
    popularScore.persist()
  ...
  def similarityScore(g: Graph[String, String], candidates : Array[Long], mention_keywords:Set[String], graphNbrs : VertexRDD[Set[String]]) : VertexRDD[Double]= {
    ...
    // Also normalize weight by the degree of the neighbour
  ...
  def similarityScore(g: Graph[String, String], candidates : Array[Long], textContext : String, sennaConfig: SennaConfig) : VertexRDD[Double]= {
    println("calculating similarity (context) score")
    ...
    // Options : get two hop neighbrs Assign weights to 1 hop neighbours  vs second hop neighbours
    // Also normalize weight by the degree of the neighbour
    val candidate_keywords : VertexRDD[Set[String]] =  NodeProp.getOneHopNbrLabels(g).filter(v=> candidates.contains(v._1))
    ...
    println("Most similar nodes with at least 1 context match")
    keywords_with_similarity_score.foreach(v=> if(v._2._2 > 3.0) println(v._1, "similarity score" , v._2._2, v._2._1))
